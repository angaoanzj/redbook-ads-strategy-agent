...
> **作业交付文档之三**（真实品牌案例 · 全功能测试 · 准确性与实用性）。  
> 同套交付：[使用说明书](./USER_GUIDE.md)｜[技术架构](./TECHNICAL_ARCHITECTURE.md)｜[后续优化方向](./OPTIMIZATION_ROADMAP.md)  
> 细粒度来源台账（测试契约用）：[QUARTET_DATA_PROVENANCE.md](./QUARTET_DATA_PROVENANCE.md)

## 1. 测试目标

用真实品牌「曲奇四重奏」跑通 Agent 全链路，回答：

1. LLM 在 Agent Loop 里能否稳定收敛（步数、修复轮、工具拒绝后自愈）？
2. 护栏（工具参数校验 + 输出契约 + 数字溯源）能否拦住数字错误与编造？
3. 输出有多少可直接落地，多少必须人工核验——**准确性与实用性**如何？
...
| 持久化 | `data/xhs_knowledge.db`（证据库 + FTS5 + paid_metrics）、`data/agent_state.db`（会话/checkpoint/缓存/反馈） |
...
| 覆盖面 | 文件 | 方法数 |
| --- | --- | --- |
| 技术架构文档契约 | `test_architecture_docs.py` | 2 |
...
| 高峰时段分桶（北京时间 11–14 / 16–20 等） | 1/4 | 口径=笔记 `published_at`→北京时间发布量，不是平台曝光高峰；需用聚光分时报表确认 |
| 竞品共性与内容缺口 | 1 | 对标样本有限时广告标识与投放时长必须人工打开原笔记核验 |
| 8 位达人的粉丝数与报价 | 3 | 公开样本估算/演示值，下单前必须蒲公英复核档期与异常粉 |
| 聚光定向标签与人群包 | 2 | `tag_status` 已注明需在聚光后台核对标签可用性 |
| 热搜/趋势词跟进 | 6 | 案例内 4 词可评分；5118 实时热度靠 `scripts/fetch_keyword_heat.py`，未配 Key 或未导入时不得称作小红书官方热搜；无任何趋势证据时才标「待接入数据源」 |
| 加权 CPC/CPM/CTR（近 N 天折合月） | 1 | 来自品牌投流月度表加权；当前月无导出行时回看/回退最近有数月（如 2026-05），需确认原表导出口径 |
...
## 附录B：曲奇四重奏数据来源摘要（准确性边界）

| 来源 | 用途 | 等级/边界 |
| --- | --- | --- |
| 用户提供《数据需求.xlsx》（产品 / 投流 / 内容月度） | 基准 CPC/CPM/CTR、自然内容历史 | `C` 用户导入；非竞品后台 |
| 公开官网 / 品牌局 / 商场页 | 品牌档案核验 | 公开观察；宣传口径不作经营审计 |
| `examples/cookie_quartet_full_case.json` | 满证据复现输入 | 案例打包；CVR 等演示补全带 `is_mock` |
| 给定竞品链接公开页 | 对标赞藏评 / 主题 / 广告标识 | 仅用户粘贴链接，非全站爬取 |
| Mock / 实时 Feed 模拟 | 演示与回归 | 强制 `is_mock`，**不抬高** `data_confidence` |

**实用性结论（摘要）**：预算拆分、出价护栏、关键词分层、素材阈值等有工具算术与溯源的部分可直接进内部评审；竞品真实预算、聚光定向还原、全平台大盘热搜、未 OCR 的封面合规，必须人工补证后才能上线。细目见 `QUARTET_DATA_PROVENANCE.md`。

> 说明：早期做过的「Claude/GPT 零代码 vs 代码版」对照实验材料已从本仓移除；正式交付与回归仅维护代码版 Agent。
